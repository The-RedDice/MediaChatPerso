...
// The best way to fix all these unescaped newlines is to find any physical newline followed immediately by a single quote
// Or actually I can just run a regex that finds strings like '...\n...' and replaces the \n with \\n
// But wait, it's safer to re-inject that block properly because the previous script messed up all \n.

// The block we injected starts with `case 'collection': {` and ends with `case 'download': {`
const startMarker = "      // ── /collection ──────────────────────────────────────";
const endMarker = "      case 'download': {";

const originalCode = fs.readFileSync('discord-bot/index.js', 'utf8');

const startIndex = originalCode.indexOf(startMarker);
const endIndex = originalCode.indexOf(endMarker);

const fixedCodeBlock = `      // ── /collection ──────────────────────────────────────
      case 'collection': {
...
const newCode = originalCode.substring(0, startIndex) + fixedCodeBlock + originalCode.substring(endIndex);
fs.writeFileSync('discord-bot/index.js', newCode);
